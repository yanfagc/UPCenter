...
<html>
    ...
    <nav class="navbar navbar-default navbar-fixed-top" style="min-height:40px;">
      <div class="container-fluid">
        <div class="navbar-header">
          <a class="navbar-brand" href="javascript:void(0);" style="height:40px;line-height:8px;">编辑复选框字段</a>
        </div>
      </div>
    </nav>
        ...
        <table class="table table-bordered table-hover itemstable">
          ...
          <tbody>
            ...
            <tr style="text-align:center;">
              ...
              <td style="padding:8px;">
                <c:forEach items="${checkeditems}" var="ci">
                  <c:if test="${ci eq item.itemvalue}"><c:set var="cc" value="checked=\"checked\""/></c:if>
                </c:forEach>
                <input type="checkbox" value="1" ${cc}/><c:set var="cc" value=""/>
              </td>
              <td style="padding:4px;">
  ...
  <script src="${ctx}/js/basic/field/field-editCheckbox.js<c:if test='${not empty crm}'>?${crm}</c:if>" type="text/javascript"></script>
</html>