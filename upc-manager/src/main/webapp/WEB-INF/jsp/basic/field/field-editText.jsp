...
<html>
    ...
    <nav class="navbar navbar-default navbar-fixed-top" style="min-height:40px;">
      <div class="container-fluid">
        <div class="navbar-header">
          <a class="navbar-brand" href="javascript:void(0);" style="height:40px;line-height:8px;">编辑单行文本字段</a>
        </div>
      </div>
    </nav>
    <div class="container" style="width:100%;margin-top:50px;padding-left:10px;padding-right:10px;">
      <form id="submitForm" class="form-horizontal" action="${ctx}/basic/field/save" method="POST">
        ...
        <table class="table table-bordered table-hover">
          ...
          <tr>
            <td style="text-align:right;"><s>*</s>扩展类型：</td>
            <td style="padding:4px;">
              <select name="macrotype" class="form-control input-sm myspan6">
                <c:forEach items="${macrosList}" var="macros">
                <option value="${macros}" ${record.macrotype eq macros.name?'selected="selected"':''}>${macros.remark}</option>
                </c:forEach>
              </select>
            </td>
            <td style="text-align:right;">默认值：</td>
            <td style="padding:4px;">
              <input name="defaultvalue" class="form-control input-sm myspan6" type="text" placeholder="默认值" value="${record.defaultvalue}">
            </td>
          </tr>
          <tr>
  ...
  <script src="${ctx}/js/basic/field/field-editText.js<c:if test='${not empty crm}'>?${crm}</c:if>" type="text/javascript"></script>
</html>