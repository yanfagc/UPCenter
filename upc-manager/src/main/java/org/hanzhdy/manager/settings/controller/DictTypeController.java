...
public class DictTypeController extends ApplicationController {
    ...
    public Object dataList(DictTypeParams params, HttpServletRequest request) {
        ...
        try {
            ...
            logger.error("查询字典类型数据失败，查询参数：{}, 错误信息：[{}, {}]", JSON.toJSONString(params), ex.getCode(), ex.getBizMessage());
    ...
    public Object saveUser(DictType record, HttpServletRequest request) {
        ...
        try {
            ...
            logger.error("保存字典类型信息失败，数据参数：{}, 错误信息：[{}, {}]", JSON.toJSONString(record), ex.getCode(), ex.getBizMessage());
            return ex.getStatus();
        }
        catch (Exception ex) {
